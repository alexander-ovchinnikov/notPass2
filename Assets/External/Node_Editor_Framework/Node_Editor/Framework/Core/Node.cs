...
namespace NodeEditorFramework
{
    public abstract partial class Node : ScriptableObject
    {
        ...
        public Vector2 size
        {
            get { return AutoLayout ? autoSize : DefaultSize; }
        }
        ...
        public bool isChildOf(Node otherNode)
        {
            ...
            foreach (ConnectionPort inputPort in inputPorts)
            {
                foreach (ConnectionPort connectionPort in inputPort.connections)
                {
                    if (connectionPort.body != startRecursiveSearchNode &&
                        (connectionPort.body == otherNode || connectionPort.body.isChildOf(otherNode)))
                    {
        ...
        protected List<T> getTargetNodes<T>(ConnectionPort port) where T : Node
        {
            ...
            if (IsAvailable(port))
            {
                ...
                {
                    if (connection.body is T)
                    {
                        result.Add(connection.body as T);
                    }
                }
            }
            return result;
